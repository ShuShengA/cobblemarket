...
- 交易数据保存失败时，服务器日志会输出 `CobbleMarket state save failed`，同时给在线 OP 发红字告警——看到告警请立即检查磁盘空间与文件权限。

## 交易历史 CSV 与补偿对账

模组从 beta.4 起把每笔交易**同步写入** CSV 账本（写盘不依赖自动保存，崩溃/杀进程也不丢），位置：`config/cobblemarket/history/history_<日期>_<语言>.csv`，字段为：时间（秒级）、类型（上架/卖出/下架/退还）、分类、卖家、买家、精灵/物品、价格、手续费。
...
三条同时满足 → 数据确实因服务器异常关闭而丢失 → 按 CSV 记录的价格补偿玩家。

补偿局限：CSV 只记录显示名与价格，物品的附魔/NBT 细节、精灵的 IV/性格/闪光无法恢复——只能补同物种/同物品的新货；精灵按显示名对账时不同形态可能同名，只能对到「某物种」粒度。
