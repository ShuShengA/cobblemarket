...
交易数据落盘经过 mtime 校验，失败时日志报错并给在线 OP 发「保存失败」告警。**玩家频繁进出时的告警大部分是误报**（状态无未落盘变更时跳过保存，不再提示）；真正提示时按概率排查：

1. **磁盘空间不足**（最常见）：服务器盘满了写不进去，清空间或换盘
2. **网盘同步软件锁文件**（国内服主高发）：OneDrive/坚果云/百度网盘同步着服务器目录，同步进程锁定 `.dat` 导致写入失败——**服务器目录必须排除出任何网盘同步范围**
3. **杀毒软件实时扫描**：Windows Defender 等会短暂锁文件——把服务器目录加入杀软白名单
4. **文件权限/只读**：`.dat` 被设只读，或服务器进程对目录无写权限（Linux 常见：root 启动过后换普通用户跑）
5. **磁盘硬件问题**：坏道等 I/O 错误——查系统事件日志的磁盘错误，及时换盘

**兜底保障**：模组每次保存前都会写 `.bak` 备份，服务器下次启动时自动校验——主文件损坏会从 `.bak` 自动恢复。偶尔一次告警大概率是第 2/3 类瞬时锁文件；每次都告警则优先查磁盘空间和权限。
