 ...
 */
class MeowthBlackCardItem : Item(Settings().maxCount(1).fireproof()) {
    ...
    override fun inventoryTick(stack: ItemStack, world: World, entity: Entity, slot: Int, selected: Boolean) {
        ...
        if (state.isBlackCardHolder(player.uuid)) return
        stack.setCount(0)
        player.sendMessage(
            Text.translatable("cobblemarket.card.black_not_holder_removed").formatted(Formatting.RED),
            false
        )
    }
    ...
    companion object {
        /** 判断栈是否黑卡（用于地面实体扫描） */
        fun isCard(stack: ItemStack): Boolean = stack.item is MeowthBlackCardItem

        /** 地面扫描：丢弃的黑卡实体快速消失（丢弃即消失；持有者可从喵喵银行补发凭证） */
