...
object CurrencyHandler {
    ...
    fun getBalance(player: ServerPlayerEntity): BigInteger {
        if (useCobeco) {
            return try {
                val eco = CobblemonEconomy.getEconomyManager() ?: return BigInteger.ZERO
                // cobeco 余额为 BigDecimal（可能有小数），取整向零截断，与其内部扣款粒度一致
                val raw = if (usePco) eco.getPco(player.uuid) else eco.getBalance(player.uuid)
                raw.setScale(0, RoundingMode.DOWN).toBigInteger()
            } catch (e: Throwable) {
                // 捕获 Throwable：mod 被移除时可能抛 NoClassDefFoundError
                CobbleMarket.LOGGER.error("Failed to get cobeco balance for {}", player.uuid, e)
                BigInteger.ZERO
            }
        }
        if (useCobbleDollars) {
    ...
    fun remove(player: ServerPlayerEntity, amount: Int): Boolean {
        ...
        if (useCobeco) {
            return try {
                ...
                if (usePco) eco.subtractPco(player.uuid, BigDecimal.valueOf(amount.toLong()))
                else eco.subtractBalance(player.uuid, BigDecimal.valueOf(amount.toLong()))
            } catch (e: Throwable) {
                // 捕获 Throwable：mod 被移除时可能抛 NoClassDefFoundError
    ...
    fun give(player: ServerPlayerEntity, amount: Long): Long {
        ...
        if (useCobeco) {
            return try {
                val eco = CobblemonEconomy.getEconomyManager() ?: return 0L
                if (usePco) eco.addPco(player.uuid, BigDecimal.valueOf(amount))
                else eco.addBalance(player.uuid, BigDecimal.valueOf(amount))
                amount
            } catch (e: Throwable) {
                // 捕获 Throwable：mod 被移除时可能抛 NoClassDefFoundError
