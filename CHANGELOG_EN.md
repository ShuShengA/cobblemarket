...
- **Meowth Purple Card**: a high-limit credential item — holders get a fixed borrowing limit set by the owner (default 1M), with a configurable server-wide card cap (default 20); the limit is bound to holder state, not the item (cards duplicated by item bugs are worthless); dropped cards vanish instantly and can be reissued at Meowth Bank; owners use `/market card` to give/revoke/list
- **Meowth Black Card**: a credential one tier above the purple card — limit defaults to 5M with a server cap of 5 (both configurable); applying requires already holding the purple card (hard requirement), with the same six configurable conditions as the purple card; holding both cards uses the black card's limit and fee discount; the black card icon on the right side of Meowth Bank opens the application screen; owner commands gain a card-kind argument (`/market card give <player> black`, same for revoke/list, purple by default)
- **Purple card self-application**: owners can let players apply themselves — clicking the purple card on the left of Meowth Bank opens the application screen, listing all seven conditions (asset / spending / credit / deposit / Pokedex count / clean record / application fee) with live progress; apply once everything passes, and the fee goes to the reserve pool
...
- **Purple card holder fee discount**: owners can configure a market fee discount for purple card holders (covers listing / auction settlement / buy-order fees, stacks with overdue doubling, off by default) — holding the card makes trading cheaper
- **Purple card reissue fee**: reissuing a purple card now costs a configurable fee (free by default), which goes to the reserve pool; the application screen shows the reissue fee for holders
...
- Fixed purple card apply/reissue with a full inventory: the card used to drop to the ground and vanish instantly — now the request is refused with a clear-inventory hint (checked before any fee is taken)
